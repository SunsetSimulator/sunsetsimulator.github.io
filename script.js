...
document.addEventListener('touchmove', function(e){ 
  mouse = {x:event.touches[0].pageX, y:event.touches[0].pageY};
    ...
    //if(mouseIsDown) {
      ...
      document.getElementById("darknessOverlay").style.opacity = Math.min((mouse.y-(myHeight)) / (myHeight), 1);
      ...
      document.getElementById("horizonNight").style.opacity = (mouse.y-(myHeight*5/5)) / (myHeight-(myHeight*5/5));
      ...
      document.getElementById("sky").style.opacity = Math.min((myHeight), 0.99);
      ...
      if(mouse.y > 0) {
        ...
        for(var i=0; i<clouds.length; i++) {
          clouds[i].style.left = Math.min(myWidth*(Math.pow(mouse.y,2)/Math.pow(myHeight)) * -1, 0);
        }
      //}
      ...
      if(mouse.y > myHeight/2) {
        document.getElementById("sun").style.opacity = Math.min((myHeight-mouse.y) / (myHeight) + 0.2, 0.5);
        document.getElementById("horizon").style.opacity = (myHeight-mouse.y) / (myHeight) + 0.2;

        document.getElementById("waterReflectionMiddle").style.opacity = (myHeight-mouse.y) / (myHeight/2) - 0.1;
      } else {
        document.getElementById("horizon").style.opacity = Math.min(mouse.y / (myHeight), 0.99);

        document.getElementById("sun").style.opacity = Math.min(mouse.y / (myHeight), 0.5);
        document.getElementById("waterReflectionMiddle").style.opacity = mouse.y / (myHeight) - 0.1; 
      }
...
function windowResize() {
  ...
  document.getElementById("waterDistance").style.height = 0 ;
